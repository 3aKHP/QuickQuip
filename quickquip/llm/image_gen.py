...
from quickquip.llm.config import ImageModelConfig, ImageProviderConfig
...
def _get_api_key(provider: ImageProviderConfig) -> str:
    ...
    if not api_key:
        raise LLMProviderError(f"环境变量 {provider.api_key_env} 未设置，provider {provider.id} 无法调用")
    return api_key


async def download_image(url: str, timeout: float = 20.0) -> ImageInput:
    """Download an image URL and return ImageInput. Raises LLMProviderError on failure."""
    def _fetch() -> ImageInput:
        try:
            ...
            raise LLMProviderError(f"图片下载失败：HTTP {exc.code}") from exc
        except error.URLError as exc:
            raise LLMProviderError(f"图片下载失败：{exc.reason}") from exc
        except OSError as exc:
            raise LLMProviderError(f"图片下载失败：{exc}") from exc
...
async def _openai_extract_image(data: dict) -> str:
    """Extract base64 image from OpenAI images API response (b64_json or url)."""
    items = data.get("data", [])
    if not items:
...
async def _openai_images_edit(
    ...
    for i, img in enumerate(input_images):
        ext = _ext.get(img.media_type, "jpg")
        header = (
            f"--{boundary}\r\n"
            f'Content-Disposition: form-data; name="image[]"; filename="image{i}.{ext}"\r\n'
            f"Content-Type: {img.media_type}\r\n\r\n"
        ).encode()
        parts.append(header + img.data + b"\r\n")
...
async def _minimax_images(
    ...
    *,
    ...
    if base_resp.get("status_code", 0) != 0:
        raise LLMProviderError(base_resp.get("status_msg", str(data)))
