...
def register_session_commands(on_command, Message, MessageSegment) -> None:
    ...
    async def _start_private_session(event, matcher, cmd_name: str) -> None:
        ...
        if has_resume:
            result = svc.resume_private_session(event.user_id, resume_num)
            if "error" in result:
                await matcher.finish(result["error"])
            preset_override = _parse_preset(args)
            if preset_override:
                scope_key = svc.build_chat_scope_key(event.user_id, "private")
    ...
    async def _end_private_session(event, matcher, cmd_name: str) -> None:
        ...
        result = svc.end_private_session(event.user_id, save=not no_save)
    ...
    async def _(event):
        ...
        result = svc.resume_private_session(event.user_id, archive_number)
        if "error" in result:
    ...
    async def _(event):
        ...
        _ensure_llm_bindings()
        svc = get_llm_service()
        await sessions_cmd.finish(svc.format_session_archives(event.user_id))
    ...
    async def _(event):
        ...
        deleted = svc.delete_session_archive_for_user(event.user_id, archive_number)
        if deleted:
