...
from quickquip.common.record_content import from_segments, legacy, plain, references, render, validate
...
def test_record_storage_rejects_unknown_table(tmp_path):
    ...
    with sqlite3.connect(tmp_path / "db") as conn, pytest.raises(ValueError, match="unsupported record table"):
