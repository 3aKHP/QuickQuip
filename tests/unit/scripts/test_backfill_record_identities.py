...
def test_backfill_preview_repeat_race_backup_and_match_parity(tmp_path, snapshot):
    backfill = runpy.run_path(
        str(Path(__file__).resolve().parents[3] / "scripts/backfill_record_identities.py")
    )["backfill"]
    ...
    with sqlite3.connect(path) as conn:
        assert "content_parts_json" not in [
            r[1] for r in conn.execute("PRAGMA table_info(memories)")
        ]
    def race(row):
        if row["id"] == 2:
            with sqlite3.connect(path) as conn:
    ...
    with sqlite3.connect(path) as conn:
        content, body = conn.execute(
            "SELECT content, content_parts_json FROM memories WHERE id=1"
        ).fetchone()
        assert content == raw
        for q in ["标准名", "别名", "12345", "旧名"]:
            assert matches({"content": raw}, q, snapshot) == matches(
                {"content": raw, "content_parts_json": body}, q, snapshot
            )
...
def test_backfill_repairs_only_missing_index_and_failure_exit(tmp_path):
    ...
    ident = store.add(
        "10001", "23456", "名字", "", "99999", content_parts=legacy("[CQ:at,qq=12345]")
    )
    with store._db:
        encoded = store._db.execute(
            "SELECT content_parts_json FROM quotes WHERE id=?", (ident,)
        ).fetchone()[0]
    ...
    assert script["main"](
        ["--database", "quotes", "--path", str(path), "--preview-limit", "0"]
    ) == 1
...
def test_backfill_shipped_and_standalone_help(tmp_path):
    ...
    for dockerfile in ("Dockerfile", "prod.example/Dockerfile"):
        assert any(
            line.startswith("COPY ") and script in line
    ...
    assert any(
        "Copy-Item" in line and "scripts\\backfill_record_identities.py" in line
