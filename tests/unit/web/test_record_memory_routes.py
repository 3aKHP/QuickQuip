...
def test_reference_edit_metadata_and_plain_client(client):
    ...
    with store._connect() as conn:
        before = conn.execute("SELECT content_parts_json FROM memories WHERE id=?", (ident,)).fetchone()[0]
        assert json.loads(before) == body
        assert conn.execute("SELECT qq FROM memories_member_refs WHERE record_id=?", (ident,)).fetchone()[0] == "12345"
...
@pytest.mark.parametrize("body", [{"version": 2, "parts": []}, {"version": 1, "parts": [{"type": "member", "qq": "oops"}]}, {"version": 1, "parts": [{"type": "text", "text": "a" * 4097}]}])
