...
async def test_screenshot_remember_and_literal_cq(tmp_path, monkeypatch, snapshot):
    ...
    svc = SimpleNamespace(remember_memory=lambda group, content, **kw: store.add_memory(group, content, content_parts=kw["content_parts"]))
    ...
    message = Message([MessageSegment.text("/remember "), MessageSegment("at", {"qq": "12345", "name": "旧名"}), MessageSegment.text(" 喜欢编程")])
    event = SimpleNamespace(group_id=10001, user_id=99999, message_type="group", get_message=lambda: message)
...
async def test_screenshot_quote_name_fallback_and_command_retained(tmp_path, monkeypatch, snapshot):
    ...
    monkeypatch.setattr(history, "get_sender_identity_sources", lambda scope: (snapshot.names, snapshot.index))
    reply = SimpleNamespace(user_id="12345", sender=SimpleNamespace(nickname="旧作者", card=""), message="/remember [CQ:at,name=可用名称,qq=23456,extra=ok] [CQ:image,file=x]")
    event = SimpleNamespace(group_id=10001, user_id=99999, self_id=88888, message_type="group", reply=reply, get_message=lambda: Message("/quote"))
...
async def test_quote_pure_media_rejected(tmp_path, monkeypatch, snapshot):
    store = GroupQuoteStore(tmp_path / "quotes.db")
    monkeypatch.setattr(history, "group_quote_store", store)
    event = SimpleNamespace(group_id=10001, user_id=99999, message_type="group", reply=SimpleNamespace(user_id="12345", message="[CQ:image,file=x]"), get_message=lambda: Message("/quote"))
    matcher = register(history.register_history_commands)["quote"]
    with pytest.raises(Finished):
        await matcher.handlers[0](event)
