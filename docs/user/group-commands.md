...
《杀戮尖塔》梗的显式命令版：把输入内容提炼成一张语义上最接近的卡牌/遗物名，回复一句「<名字>了」。
...
然后直接附一张图，或先引用一条群消息再发送；引用内容里的图片也会一起参与。

### 5.3 输出长什么样

机器人固定只回一句话，例如「壁垒了」。

群聊和私聊都可用，当前没有中文别名。该命令走独立限流（`sts_turmfluch` 桶），触发过于频繁时会提示稍后再试。
...
机器人可以收集群聊数据，按早/午/晚报三个时段生成群聊动态简报。该功能默认关闭，需要管理员手动开启；此外还需部署者在 `config/llm.toml` 中全局开启 `[daily_briefing]`。
...
立即生成每分钟限一次。如果当前模型输出异常中断，机器人会自动尝试级联里的下一个播报模型，不会直接把半截午报发出来。
...
拉取 MCP 镜像并重连所有 MCP 服务，完成后返回最新连接状态。
...
`awakening_extend` 只承接你刚刚明确艾特或用 LLM 前缀召唤 bot 后的一小段对话；兴趣、兜底、相关性、答疑和无聊唤醒不会让 bot 持续接后续普通聊天。

兴趣、相关性、答疑、延长和无聊唤醒触发时，bot 会携带群内最近发过的少量历史图片一起看，而不只看当前消息里的图片；显式触发（`/ai`、`@机器人`）和兜底唤醒不会带历史图片。
...
- `/llm delete_msg`
- `/llm reload`
- `/llm probe` — 探活所有 provider
- `/llm mcp reload` — 重载 MCP 连接
- `/remember <内容>`
- `/forget <关键词>`
- `/forget_all`
- `/disable <rule>`
...
- `/wordcloud [today|week|month|year]` — 生成词云
...
机器人可以收集当天的群聊，每天早上六点自动生成一篇约 2000 字的小作文风格日报，并在中午十二点发到群里。该功能默认关闭，需要管理员手动开启；此外还需部署者在 `config/llm.toml` 中全局开启 `[daily_summary]`。
