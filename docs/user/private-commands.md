...
- 引用一条消息追问时，当前提问与被引用内容会分开理解；引用里的合并转发会尽量读取其中的正文与图片。
- 上下文（AI 每次回复时能回看的最近消息条数）私聊默认 256 条、群聊默认 10 条（部署配置可改）；用 `/llm context_limit` 调整，私聊上限 256 条。
...
| `/llm context_limit <条数>`（`reset` 或 `off` 恢复默认） | 设置每次回复回看的消息条数 |
