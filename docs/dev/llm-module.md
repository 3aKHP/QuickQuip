...
- 消息中的艾特在**入口 ingestion 时**按**群合并身份索引**渲染为 `@标准身份`（引用消息、合并转发子消息同索引）；未登记成员经群成员名片缓存（`get_group_member_info`，按 群×QQ 带 TTL）退化为 `@当前群名片`，查询失败才回退 `@QQ 号` 数字形态——名片预取覆盖消息顶层 @，引用/转发子消息内未登记 @ 直接走数字形态
