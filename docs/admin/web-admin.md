...
- **日志归档** — 历史轮转日志浏览、预览与下载
- **调度器监控** — APScheduler 全部任务（节日问候、每日总结/播报、定时消息等）的只读运行时面板（job ID、trigger、next_run、last_run 时间与状态、错误详情）。调度器运行在 bot 进程内，bot 端每 30 秒把任务快照与最近执行结果写入 `data/cron_jobs.json`，本页读取该共享状态文件；群聊定时消息的增删改在「定时消息」页
