...
两个开关的计数状态按（规则, 群）隔离，只存内存、重启即重置；规则级 `probability` 只覆盖基础概率，两个开关始终跟随所在桶。`probability = 1` 搭配 `suppress_after_hit` 会出现"一回一哑"的规律交替，建议与 `probability < 1` 组合使用以保留随机感。

两点注意：覆写系统预定义桶（如 `timezone_wake`、`sts_card_le`）时整个条目需重写，`global_limit` / `user_limit` 一并写全；`llm_chat` 与唤醒类桶的概率调低后，bot 对直接 @ 也会偶发沉默，仅建议在确实需要全局静音降噪时使用。
