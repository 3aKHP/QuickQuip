...
Web Admin 当前提供 27 个标签页（前端使用 vue-router 4 hash 模式，深链接形如 `/ops/#/stats`）。前端使用响应式设计、亮色/暗色主题切换，以及一套以 QQ 蓝为主色、青/琥珀为辅助色的设计 token 系统：氛围层（侧栏/状态条/抽屉/Toast）采用半透玻璃浮于克制动效的粒子光场之上，内容区（卡片/表格/表单）保持实色以保证可读性；全局缓动为 linear/steps 机械风格，换页时顶部有一道光带横扫。
