  ...
  <div>
    ...
    <div v-else class="server-grid">
      <UiCard v-for="s in servers" :key="s.id" padding="md" shadow="sm">
        <div class="s-head"><div class="s-title"><span class="s-name">{{ s.id }}</span><span v-if="s.detail" class="s-detail">{{ s.detail }}</span></div><div class="s-status"><span class="dot" :class="dotClass(s)" /><span class="muted">{{ statusText(s) }}</span><UiInfoTip v-if="s.runtime_available === false" text="「状态未知」表示 Web 后台尚未拿到 bot 进程写入的状态（bot 可能未运行）。" /></div></div>
        <div class="s-meta"><UiTag size="sm" variant="info">{{ s.transport }}</UiTag><UiInfoTip text="连接方式：stdio=本地子进程、docker=容器、http=Streamable HTTP 单端点、sse=经典 HTTP+SSE 长连接。" /><UiTag v-if="s.era_tag" size="sm" variant="info">{{ s.era_tag }}</UiTag><UiInfoTip v-if="s.era_tag" text="双协议纪元标签：前者为配置的协商模式（legacy/auto/modern），后者为实际协议纪元；两者一致时只显示一个。仅 http transport 生效，由 config/llm.toml 的 negotiation 键控制。" /><span v-if="s.negotiated_protocol_version" class="muted">v{{ s.negotiated_protocol_version }}<UiInfoTip text="握手协商后实际使用的 MCP 协议版本，连接成功后才有值。" /></span><span class="muted">{{ s.tool_count }} 个工具</span></div>
