<template>
  <div>
    ...
    <div v-if="jobs.length" class="refresh"><UiIcon name="RefreshCw" :size="12" /><span>每 30 秒自动刷新，快照时间 {{ snapshotText }}</span><UiInfoTip text="数据来自 bot 进程定期写入的状态文件：生产部署下 Web 与 bot 是两个进程，只能读文件。" /><span v-if="snapshotStale" class="stale">快照超过 5 分钟未更新，bot 进程可能未运行</span></div>
  </div>
</template>
