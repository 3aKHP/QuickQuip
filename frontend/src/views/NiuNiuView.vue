<template>
  <div>
    ...
    <UiCard padding="md" shadow="sm" accent="cyan" class="section">
      <h3 class="st section-title">用户查询</h3>
      ...
      <div v-if="userDetail" class="user-detail">
        <div class="acct-info"><span class="al">QQ</span><span class="mono">{{ userDetail.uid }}</span><span class="al">长度</span><strong>{{ userDetail.length }} cm</strong><span class="al">总排行</span><span>第 {{ userDetail.rank_natural }} 名</span><span v-if="userDetail.rank_length > 0" class="al">长度排行</span><span v-if="userDetail.rank_length > 0">第 {{ userDetail.rank_length }} 名</span><span v-if="userDetail.rank_depth > 0" class="al">深度排行</span><span v-if="userDetail.rank_depth > 0">第 {{ userDetail.rank_depth }} 名</span><span class="al">绝对值<UiInfoTip text="按长度的绝对值排名：长度可为负，此榜只看大小不看正负。" /></span><span>第 {{ userDetail.rank_absolute }} 名</span><span class="al">打胶运势<UiInfoTip text="每日自动重掷的运势系数，影响当日打胶的长度收益；可在此手动修正。" /></span><span class="luck-val" :style="{ color: luckColor(userDetail.luck) }">{{ luckText(userDetail.luck) }}</span><span class="al">击剑运势<UiInfoTip text="每日自动重掷的运势系数，影响当日击剑对战的数值结算；可在此手动修正。" /></span><span class="luck-val" :style="{ color: luckColor(userDetail.fence_luck) }">{{ luckText(userDetail.fence_luck) }}</span></div>
        ...
        <h4 class="st section-title" style="margin-top:var(--qq-gap-md)">操作记录（最近 30 条）</h4>
        ...
        <UiEmpty v-else compact icon="FileText" title="暂无操作记录" />
      </div>
      <p v-else-if="userSearched" class="muted" style="margin-top:12px">未找到该用户</p>
    </UiCard>

    <UiCard padding="md" shadow="sm" accent="accent" class="section">
      <h3 class="st section-title">文案模式管理</h3>
