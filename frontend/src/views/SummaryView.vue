  ...
  <div class="sum-view page-view-fill">
    ...
    <UiCard v-if="health" padding="md" shadow="sm" class="health-card">
      ...
      <table v-if="healthRows.length" class="health-table">
        <thead><tr><th>链路<UiInfoTip :text="linkTip" /></th><th>调用</th><th>接受<UiInfoTip :text="acceptTip" /></th><th>异常<UiInfoTip :text="outcomeTip" /></th><th>均耗时</th><th>成本</th></tr></thead>
        <tbody>
...
const linkTip = '三条生成链路：日报 = 每日群聊总结；简报 = 早/午/晚播报（llm.toml [daily_briefing]，直接发群、不在下方归档列表）；周月报 = 周报与月报共用同一链路统计。'
const acceptTip = '该跳响应被采纳的判定：正文非空且模型正常结束（finish_reason 为 stop/end_turn/stop_sequence/eos 之一）；级联中未被采纳才会尝试下一跳。'
