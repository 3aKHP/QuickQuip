...
def register_history_commands(on_command, Message, MessageSegment) -> None:
    ...
    async def _(bot, event):
        if _is_private_chat(event):
            await profile_cmd.finish(MessageSegment.text("该命令仅支持群聊"))
        _ensure_llm_bindings()
        svc = get_llm_service()
        if not svc.config.is_available:
            await profile_cmd.finish(MessageSegment.text("LLM 功能未启用，无法生成人物志"))

        target_user_id = _extract_at_target(_raw_message_text(event), event.get_message())
        ...
        target_name = identities.snapshot(group_id).name(target_user_id)
        ...
        if not provider:
            await profile_cmd.finish(MessageSegment.text("LLM provider 未配置"))
        ...
        try:
            ...
            await profile_cmd.finish(MessageSegment.text(f"人物志生成失败：{exc}"))
    ...
    async def _(event):
        if _is_private_chat(event):
            await find_cmd.finish(MessageSegment.text("该命令仅支持群聊"))
        keyword = _strip_command_name(str(event.get_message()).strip(), "find").strip()
        if not keyword:
            await find_cmd.finish(MessageSegment.text("用法：/find <关键词>"))
        group_id = event.group_id
        now = time()
        ...
        for m in shown:
            ...
            text = render(legacy(m.get("text", "")), snapshot)
            if len(text) > 50:
                text = text[:50] + "…"
            lines.append(f"[{ts}] {snapshot.name(m.get('user_id'), m.get('sender', ''))}: {text}")
        await find_cmd.finish(MessageSegment.text("\n".join(lines)))
    ...
    async def _(event, bot=None):
        ...
        if seq_match:
            ...
            q = group_quote_store.get_by_seq(group_id, seq, identity_snapshot=snapshot)
            if q is None:
        ...
        if by_match:
            ...
            is_qq = query.isdigit()
            user_ids = [query] if is_qq else _resolve_sender_candidates(group_id, query, sources)
