...
from quickquip.adapters.nonebot.record_content import prepare_body
...
def register_memory_commands(on_command, Message, MessageSegment) -> None:
    ...
    async def _(event):
        if not _allow_scope_management(event):
            await forget_cmd.finish(MessageSegment.text("仅管理员可执行此操作"))
        keyword = _strip_command_name(str(event.get_message()).strip(), "forget")
        if not keyword:
            await forget_cmd.finish(MessageSegment.text("用法：/forget <关键词>"))
        _ensure_llm_bindings()
        svc = get_llm_service()
        try:
    ...
    async def _(event):
        if not _allow_scope_management(event):
            await forget_all_cmd.finish(MessageSegment.text("仅管理员可执行此操作"))
        _ensure_llm_bindings()
        svc = get_llm_service()
    ...
    async def _(event, bot=None):
        if _is_private_chat(event):
            await tell_cmd.finish(MessageSegment.text("该命令仅支持群聊"))
        to_user_id = None
        segments = []
        at_found = False
        for seg in event.get_message():
            ...
            if seg_type == "at" and not at_found and QQ.fullmatch(str(data.get("qq", ""))):
                to_user_id = str(data["qq"])
                at_found = True
            elif at_found:
                segments.append(seg)
        if not to_user_id:
        ...
        try:
            ...
            await tell_cmd.finish(MessageSegment.text(str(exc)))
        await tell_cmd.finish(MessageSegment.text("留言已存，TA 下次发言时会收到"))
    ...
    async def _(event):
        if _is_private_chat(event):
            await tells_cmd.finish(MessageSegment.text("该命令仅支持群聊"))
        pending = offline_message_store.list_pending_for(event.group_id, event.user_id)
        if not pending:
            await tells_cmd.finish(MessageSegment.text("没有待接收的留言"))
        lines = [f"有 {len(pending)} 条留言等着你："]
        snapshot = identities.snapshot(event.group_id)
        for m in pending:
            lines.append(m.format_display(snapshot))
        await tells_cmd.finish(MessageSegment.text("\n".join(lines)))
    ...
    async def _(event):
        if _is_private_chat(event):
            await untell_cmd.finish(MessageSegment.text("该命令仅支持群聊"))
