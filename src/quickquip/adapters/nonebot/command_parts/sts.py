...
def register_sts_commands(on_command, Message, MessageSegment) -> None:
    ...
    「故障机器人」的五字梗。两命令同构，仅 prompt/校验/限频桶不同。
    """
    ...
    defectify_cmd = on_command("defectify", aliases=DEFECTIFY_ALIASES, priority=10, block=True)

    @defectify_cmd.handle()
    async def _(event):
        if not rate_limiter.allow(DEFECTIFY_RATE_LIMIT_KEY, event.user_id):
        ...
        with bot_action_trace(
            ...
            rule_name=result.get("rule_name", DEFECTIFY_RULE_NAME),
            chat_type=chat_type,
            group_id=getattr(event, "group_id", ""),
            user_id=event.user_id,
            incoming_message_id=str(getattr(event, "message_id", "") or ""),
