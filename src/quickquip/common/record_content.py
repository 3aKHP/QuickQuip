...
def validate(body, max_length=4096):
    if (
        ...
        or not isinstance(body.get("parts"), list)
    ):
    ...
    for part in body["parts"]:
        ...
        if kind == "text" and isinstance(part.get("text"), str):
            ...
            if (
                not isinstance(part.get("usage", "mention"), str)
                or part.get("usage", "mention") not in {"mention", "identity"}
...
def from_segments(message, command=None):
    ...
    for segment in message:
        ...
        data = (
            ...
            else getattr(segment, "data", {})
        )
        if kind == "text":
            text = str(data.get("text", ""))
            if command_pending:
                text, count = re.subn(
                    r"^\s*[/!]?" + re.escape(command) + r"(?=\s|$)\s*", "", text, count=1
                )
...
def legacy(text):
    ...
    for match in CQ.finditer(text):
        ...
        body = from_segments(
            [{"type": match[1], "data": {k: unescape(v) for k, v in data.items()}}]
        )
        if not body["parts"]:
            continue
...
def decode(content, encoded=None):
    if encoded:
        try:
            return validate(
                json.loads(encoded) if isinstance(encoded, str) else encoded,
...
def render(body, snapshot=None):
    ...
    for part in body["parts"]:
        ...
        if kind == "text":
            ...
            name = (
                snapshot.name(part["qq"], part.get("name", ""))
                if snapshot
                else part.get("name") or "QQ" + part["qq"]
            )
