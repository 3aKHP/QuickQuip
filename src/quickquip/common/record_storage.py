...
def migrate(conn, table):
    ...
    try:
        ...
        if "content_parts_json" not in {r[1] for r in conn.execute(f"PRAGMA table_info({table})")}:
            raise
    conn.execute(
        f"CREATE TABLE IF NOT EXISTS {table}_member_refs "
        f"(group_id TEXT NOT NULL, record_id INTEGER NOT NULL, "
        f"qq TEXT NOT NULL, PRIMARY KEY(record_id, qq))"
    )
    conn.execute(
        f"CREATE INDEX IF NOT EXISTS idx_{table}_member_refs "
        f"ON {table}_member_refs(group_id, qq, record_id)"
    )
    conn.execute(
        f"CREATE TRIGGER IF NOT EXISTS delete_{table}_refs "
        f"AFTER DELETE ON {table} BEGIN "
        f"DELETE FROM {table}_member_refs WHERE record_id=OLD.id; END"
    )
...
def save_parts(conn, table, record_id, scope, body):
    checked_table(table)
    conn.execute(
        f"UPDATE {table} SET content_parts_json=? WHERE id=?",
        (json.dumps(body, ensure_ascii=False), record_id),
    )
    ...
    conn.executemany(
        ...
        [(str(scope), record_id, qq) for qq in references(body)],
    )
