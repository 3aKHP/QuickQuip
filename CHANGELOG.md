...
本版围绕**定时总结输入层与管理后台可用性**：群月报改为分周预算组装全量输入，日报接入与周报/月报一致的压缩序列化；总结页健康度收敛为链路级汇总并新增生成日志；管理后台各页面全面补充悬浮说明。分段交付开关拆分为中间轮/最终轮两域独立控制，MCP 文本资源正文恢复有界交付。

**升级说明**：月报配置项 `monthly_report.sample_per_day` 移除（按天采样退役），可改配新键 `monthly_report.input_char_budget`（默认 240000）；`/llm delivery` 命令升级为三域语法 `/llm delivery intermediate|final|all <on|off|reset>`，存量按群开关值自动迁移，无需人工处理。

### 变更

- 群月报改为分周预算组装全量输入：上月全量消息经压缩序列化后按自然周公平分配字符预算（平静日整日保留，高活跃日优先用满剩余预算，放不下时等距抽稀），聊天记录带 `【第N周 MM-DD–MM-DD】` 周节标题。`monthly_report.sample_per_day` 移除，新增 `monthly_report.input_char_budget`（默认 240000，最小 8000）；信息覆盖显著高于按天抽稀口径，窄窗模型由级联每跳截断兜底。
- 群日报聊天记录接入与周报/月报相同的压缩序列化器（按天分节、同分钟连发合并、相同消息折叠计数、URL 只留域名并标记机器人发言），忙日输入 token 显著下降；日报/周月报 system prompt 补充成文结构建议，格式说明与注入防护文案抽为共用段，三线口径一致。
- 总结页「生成健康度」收敛为按链路一行汇总（调用/接受率/异常构成/均耗时/成本），报文详情新增「生成日志」折叠区（级联各跳时间点、模型、耗时、token 与 finish_reason）；每次生成携带 run_id 精确归因，历史报文按生成时间窗自动兜底推算。
- 分段交付开关拆分为两枚独立开关：「中间轮发送」（多轮工具回复中非最终轮的过程正文照常发出）与「最终轮分段」（最终长回复按自然段拆成多条）。`/llm delivery` 改为 `/llm delivery intermediate|final|all <on|off|reset>`，`/llm delivery status` 查看两域状态；Web Admin 群设置同步提供两枚开关。原 `agent_delivery_enabled` 配置键仍可读取（按两域同值映射），存量按群开关值自动迁移。

### 新增

- 诊断页「最近动作」历史新增「清空历史」按钮（排队中与执行中的动作保留）。
- 管理后台全面补充悬浮问号说明：各管理页面的术语、指标口径、状态与操作按钮旁新增「?」悬浮解释，悬停或点击即可查看。
- 部署脚本 deploy-v4 统一 POSIX 风格参数并引入版本身份留痕：发布记录带版本号与构建时间戳，`-m` 备注写入运维日志；Bash 与 PowerShell 行为一致。
- 新增社区协作配套文件：`SECURITY.md`（中英对照漏洞上报渠道与支持版本口径）、行为准则、issue 模板（Bug 报告/功能提案/开发备忘）与 PR 模板（内嵌变更分级与验证清单，发布 PR 专用模板覆盖冻结与回灌步骤）。

### 修复

- 修复机器人自发消息可能再次执行群命令或管理命令的问题，群自发消息仅进入聊天归档。
- 统一单次模型请求中用户图片与工具图片的媒体预算及内容去重，优先保留当前消息图片。
- 修复聊天归档初始化失败后的自动恢复，历史回灌明确报告写入失败并支持安全重试；Windows 懒人包随附回灌脚本并校验打包后的迁移入口。
- 总结健康度按正文接受结果统计各链路尝试，保留丢弃调用的成本并标识历史未知结果；修正归档概览的天数统计。
- 修复经 GitHub 等 MCP 工具读取文件正文时被内容安全边界整体扣留的问题：内联文本资源（文本类 MIME）的正文有界交付给模型，二进制内容与链接维持扣留。
...
- LLM 工具对话按执行轮持久化模型正文、工具调用与结果、发送回执，跨轮重放支持 OpenAI、Claude、Gemini 的结构化工具消息及可用的原生内容。管理 API 提供 Loop、Turn、工具状态与交付回执详情。
- 可选逐 Turn 分段交付：每次模型响应的普通正文可先于工具执行发出，长回复按自然段拆分，发送间隔和条数上限可配置。全局默认关闭，可通过 `/llm delivery on/off/reset/status` 或管理后台“群设置”按群、私聊会话覆盖；群聊、私聊、定时消息和无聊唤醒使用统一交付出口。
- 请求输入预算门禁：结合模型上下文窗口、输出预留和估算余量检查实际请求；支持内置常见模型窗口表、显式模型容量，以及 provider 级请求和重放预算覆盖。

### 变更

- 历史重放按预算逐级精简：先剥离原生思考内容，再移除原生副本、收紧工具结果，必要时使用文本档案、摘要或逐出最旧记录。原生内容和工具结构计入请求预算；首轮超限时缩小历史窗口并重建一次。
- 撤回与引用通过发送回执定位分段；私聊归档和恢复保留完整执行轮及记录身份。旧版会话记录在首次打开数据库时自动迁移，执行记录按保留天数、轮数和字节上限清理。
- 部署模板补充依赖文件预检、打包排除和旧部署残留清理；本地巡检二维码渲染可选择具备所需依赖的项目或系统 Python。

### 修复

- 旧版数据库升级时串行执行配置列迁移，避免 Bot 与管理后台并发首次启动因重复加列而使存储不可用。
- 工具历史按当前敏感词表检查，命中内容或输出过滤替换态使用安全文本档案；预算降级持续保留该政策，避免跨轮重新带入已屏蔽原文。
- 群聊现场消息在首轮请求和长上下文缩窗重建中复用同一快照，并按最新历史去重，防止重复消费后丢失近期话题。
- 管理后台对话删除展示排队、执行和失败状态，仅在确认删除后提示成功并刷新关联消息；等待超时可继续查询原任务。
- 分段交付保留请求级发送出口，清除段尾空白并跳过空消息；同会话并发触发的无记录回复保持可见，发送后的消息 ID 正常回填。
- 交付统计只计入携带可信消息 ID 的确认发送；发送失败停止后续生成与工具执行，工具循环达到安全上限时保留中断事实。进程重启补齐未关闭执行轮的终态，恢复过程不重复执行工具。

## [1.14.3] - 2026-09-06

本版主题是**管理后台可用性与自动回复降噪**：四个工作台式页面占满窗口高度、用量页统计区重排、全站术语悬浮解释，以及为所有非命令触发的自动回复引入可配置的触发概率。

### ✨ 新增 (Added)

- 自动回复概率机制：所有非命令触发的自动回复（文字规则、语境规则、时区回复、被动「xxx了」、复读检测、乖女链、接龙、内置游戏、唤醒、显式 LLM 回复）支持配置触发概率 `probability`（0–1，缺省行为不变）。命中后先掷骰再回复，未掷中时保持沉默，不消耗限流配额、不花费 LLM 判定成本；概率可按限流桶或按规则配置，规则级覆盖桶级。
- 两个可选的方差驯化开关（桶级配置，默认关闭）：`suppress_after_hit` 防连发（同一规则同一群命中后接下来 N 次强制沉默）、`pity_step` 保底步进（连哑越多概率越高），收窄纯随机的连发/连哑方差；状态按（规则，群）隔离、私聊按用户隔离，只存内存。
- `chat_rules.toml.example` 按推荐密度预置默认概率，新部署开箱即用：时区回复降半、被动「xxx了」压至四成、bot 跟读复读降四成、泛匹配梗（如新三国系列的日常高频词）压低、特定台词梗保持高响应；斜杠命令、游戏操作、直接 @ 对话不受影响。
- 管理后台全站新增「?」悬浮解释（可复用 `UiInfoTip` 组件，hover/聚焦/点击显示，Esc、点击外部或滚动关闭，气泡不被页面边缘裁切）：覆盖「用量」的轮次信封/纪元窗口/覆盖率等术语、「群 LLM 设置」的触发方式与历史条数、「唤醒管理」的兜底/无聊/阈值参数、「记忆」的 scope 与置信度、「诊断」的风险提示、「调度器」「金币」「牛牛」「对话日志」等页的专有概念。

### 🔧 变更 (Changed)

- 管理后台「用量」页顶部统计区重排：全局 KPI（成本/请求/耗时/缓存/未定价）与「每轮均值与覆盖率」拆分为两区，均值区以紧凑行 + 覆盖率进度条呈现，消除数值换行与副文案孤字悬挂；「规则开关」「人格管理」页头补充说明字幕，「总结」页列表增加已发布/未发布状态图例。

### 🐛 修复 (Fixed)

- 管理后台四个工作台式页面在 PC 端现在占满窗口可用高度，不再固定在最小高度导致下半部分大面积留白。涉及页面：「配置」「资料」「对话日志」「总结」；窄窗口与移动端布局保持原有表现。
...
- 定时消息「仅一次」的触发时间输入从单一日期时间控件拆为日期+时间两个选择器：中文环境下原控件空值占位符是「yyyy/mm/日 --:--」混合格式，用户会误以为输入异常；拆分后提交语义与未来时间校验不变。
- 调度器监控补齐后台维护任务的可观测性：持久化自动保存、Web 管理状态同步、操作队列三个任务此前不记录执行结果，页面「上次执行」长期空白——现在与业务任务同标准记录成功/失败；「未执行」状态显式标出，不再与「无记录」混淆。
- 调度任务的最近执行结果改为跨重启保留：bot 重启后监控页「上次执行」不再清零（从既有共享状态文件恢复，无需新文件），停机期间被删除的任务不残留；非 UTF-8 编码或被手工改坏的状态文件按损坏优雅处理，不再中断调度或使监控页报错。

## [1.14.0] - 2026-09-05

本版主题是**前缀缓存与上下文经济**：让 LLM 会话的提示词前缀跨轮字节稳定，各平台的自动前缀缓存从结构性失效转为可稳定命中；同时把上下文的每个组成部分（信封、历史窗口、媒体、现场氛围）拆成独立预算并逐一计量。上下文可用量从约 0.6k token 提升到数千 token，属质量决策——历史上下文的年化成本本就不足挂齿，此前真正的代价是模型每轮都在失忆。配套七张用量账本，每段上下文的成本与利用率在看板上一目了然。

> *该优化面向 provider 侧按「最长公共前缀」自动命中的隐式缓存（DeepSeek、Kimi、MiniMax 等）：前缀逐字节稳定后，跨轮命中从结构性失效转为稳定发生。Gemini 的隐式缓存只在旧请求整体作为新请求前缀（相同或尾部追加）时才命中，而跨轮对话天然尾部发散——本版对其跨轮命中暂无手段。部署前建议先核实所用 provider 承诺（或实测）的缓存 TTL，纪元冷场阈值 `epoch_cold_idle_seconds` 宜设在不高于该 TTL 的水平：超出 TTL 只会让注定全价的轮次继续背满窗口，而略低于 TTL 至多损失几次本可命中的机会。*

### 🔧 变更 (Changed)

- LLM 会话的 system prompt 完全静态化：当前时间、节日提示、对话参与成员、持久记忆与词表命中从系统提示词移出，改由每轮 user 消息头部的【轮次上下文】信封携带（时间感知、节日人格、定时任务日期推算行为不变）；此前分钟级时间戳把 Kimi/DeepSeek 等平台的跨轮前缀缓存钉死在平台最小值，移出后稳定前缀显著变长、跨轮缓存命中率提升。用量看板新增「轮次信封」卡片，单列该段每轮全价 token（构建期估算口径）。
- LLM 短期会话从「10 行滚动窗口」改为「会话纪元」机制：每个 群×provider×model 键维护只追加的读取锚点，窗口随对话增长（默认最长 64k token 估算，冷场超过 5 分钟且窗口超 5k 时缩回 4k），纪元内提示词前缀逐字节稳定——此前每轮首条位移让 DeepSeek/Kimi/MiniMax 的自动前缀缓存结构性失效，现在暖轮可稳定命中，且可用上下文从约 0.6k token 提升到数千 token。`/llm context_limit <n>` 语义变为「该群退化为保留最新 n 行的滚动窗」，`reset` 恢复纪元自动管理；`history_max_messages_per_group` 配置废弃（存储裁剪改由纪元锚点驱动，统一上限 2048 行）。`clear_context` 现在连同纪元锚点三件齐清，`/llm persona use` 会按冷场水位前移锚点。用量看板新增「纪元窗口」卡片，单列 history 段每轮 token 估算。
- LLM 媒体与合并转发缓存策略：合并转发文本封顶 4000 字符（超出硬切并标注「已截断」），转发图片不再作为图片本体附带（视觉模型同样不附），消除 trace 中 23k 级转发尖峰与 provider 413 报错；非视觉模型的图注以文本身份落库（`[图片 N 张：…]`），下一轮历史原样复现、前缀缓存不被破坏；provider 图片下载按实例缓存（TTL 10 分钟），工具循环与退避重试不再重复下载同一图片；用量看板新增「图片附件」卡片，单列每轮实际附图数。
- LLM 群聊的群内近期发言从「全量混入上文」改岗为独立【现场】补丁段（增量语义 + 800 token 预算 + 按 message_id 自动去重 history 与当前触发消息），尾巴顺序定型【轮次上下文】→【上文】→【现场】→【当前提问】——补丁不再逐轮全价重复计费，模型也能分清对话与氛围；被动唤醒「看见近期图」不受增量收窄（图源仍走全量快照）；无聊唤醒与定时任务开始落库结构化配对行（【自动唤醒】/【定时消息】摘要，不抽自动记忆，不以伪 QQ 身份渲染），history 不再出现有答无问的孤行；用量看板新增「现场补丁」账本卡片（AVG 即预算利用率）。

### 🐛 修复 (Fixed)

- 修复用量账本 claude 协议行的口径错标：`input_tokens` 列存的原始上报值是 exclusive（不含缓存读写），标签却恒写 inclusive——朴素命中率对 claude 行可破 100%、SQL 的 exclusive 分支永远走不到；标签改按协议派生并一次性 backfill 全部历史行，看板聚合数值不变。
- 牛牛大作战的未注册指引与用户文档命令表缺少命令斜杠（「发送 注册牛牛」应为「发送 /注册牛牛」，排行命令的私聊提示同病），照提示打字无法触发命令；12 处消息提示、随包文案预设与 `docs/user` 全部牛牛命令表已对齐。
...
[Unreleased]: https://github.com/3aKHP/QuickQuip/compare/v1.15.3...HEAD
