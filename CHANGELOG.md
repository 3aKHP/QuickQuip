...
- **每日运势改为对数对称分布**：`lg(运势) ~ N(0, 1)`，中位数=1，无硬上下限，极端值（1e-9 或 1e9）虽罕见但可能出现
- **运势拆分为打胶运势与击剑运势**：两个独立每日掷骰（各自 `luck_sigma` 可调），CD 追踪器线程安全且自动清理过期条目
- **牛头人/魅魔数值对称化**：权重均提升至 30（选中率 ~19%），阈值对齐至 50，吞噬窃取比例上调至 0.3
- **打胶增长率移除阻尼因子**：长度变化完全正比于 `abs(origin)`，大长度获得更大的绝对反馈，decay 系统独立提供回归均值
- **新增群命令**：`打胶运势` / `击剑运势` 查看今日运势值及评价
