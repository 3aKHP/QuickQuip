...
- **牛牛文案模式切换**：新增 TOML 驱动的多套文案预设系统（`config/niuniu_text.toml` / `config/niuniu_text_safe.toml`），支持 `default`（原版）与 `safe`（和谐版）两套内置模式；群管理员可通过 `/牛牛文案 [模式名]` 按群切换，部署者可扩展自定义文案
- **牛牛 RPM 限流**：新增 `glue_rpm_limit`/`fence_rpm_limit`/`rpm_window_seconds` 三个配置项，可按群独立限制打胶/击剑的每分钟请求频率
- **Web Admin 牛牛面板文案管理**：牛牛管理页新增"文案模式管理"卡片，可查看可用模式、为指定群组设置文案模式，以及查看所有已设置非默认模式的群组

### 修复

- **前端移动端适配全面翻新**：修复 iOS Safari 输入框自动缩放（全局输入字号 13→16px）；所有表格视图支持横向滑动；移动端顶栏新增登出按钮；添加刘海屏安全区域适配（`safe-area-inset`）；toast 通知在移动端移至顶部避免被虚拟键盘遮挡
- **前端响应式断点统一**：消除碎片化断点（720/760/780→767），定义 4 个标准设计令牌
- **配置编辑页布局优化**：缩窄两侧面板，编辑器头部压缩为单行，增大代码编辑区域实际可用面积
- **总结页行高修复**：移动端单列布局下行高压缩导致按钮不可见的问题
- **配置列表无法滚动**：修复左侧配置文件列表 `overflow: hidden` 阻断滚动
...
- **引用合并转发识别不稳**：引用消息中的合并转发现在会递归展开节点和图片，并保留发言者身份信息，避免 LLM 仅看到占位外壳
- **引用身份串台**：A 引用 B 的消息向机器人提问时，提示词现在明确区分“当前提问者”和“引用发送者”，减少把 B 误判为当前发言者的情况
- **引用机器人自身发言识别错误**：当群友引用机器人自己的消息时，提示词现在会明确标记“机器人自己”，不再套用群资料里的普通人物身份
- **每日播报异常截断**：午报/早报/晚报生成提高输出 token 上限，并在遇到 `MAX_TOKENS` 等非正常结束时继续尝试级联模型
- **牛牛负数回归过冲**：负数长度在回归放大时不再穿过 0 反向冲出，向内/向外变化的倍率调整改为围绕 0 的距离进行判定

## [1.4.2] - 2026-05-09

### 变更

- **配置与路径收口**：LLM 与 generation 配置加载共享环境变量展开、字典读取和布尔解析 helper，剩余 `config/*.toml`、`data/*.db`、资料目录路径集中到公共常量
- **跨层路径整理**：Web route、消息管线与 LLM 运行时改为从统一路径模块取值，减少散落硬编码

### 修复

- **引用合并转发回捞失效**：修正 `get_forward_msg` 的参数路径，恢复引用合并转发时的内容抓取与相关回归测试
...
- **共享状态所有权收口**：统一 `GameScores` 的实例来源，避免同一持久化文件被多份运行时缓存同时写入
- **SQLite 生命周期补齐**：`OfflineMessageStore` 与 `GroupQuoteStore` 增加显式关闭路径，并在消息管线里统一托管
- **xdist 测试兼容性**：将持久化 store 改为惰性初始化，减少并行测试下的导入期副作用与资源争抢
...
- **牛牛击剑事件系统**：7 种击剑事件（critical 暴击、glancing 擦边、reversal 翻盘、slip 滑倒、draw 平局、dominate 牛头人支配、succubus_devour 魅魔吞噬），各事件独立伤害倍率和文案
- **魅魔吞噬真实窃取**：succubus_devour 不再只是大倍率伤害，改为窃取对手长度（min(|a|,|b|)×15%）转移给吞噬方，输家损失 ×1.5
- **牛头人支配**：原"吞噬"事件拆分，牛头人侧改为"支配"（大倍率伤害），文案和机制与魅魔吞噬区分
- **无牛牛目标击剑事件**：对未注册用户击剑触发加权随机事件（拒绝/强制注册/自伤），不再是单一拒绝文案
- **机器人击剑**：使用 `/击剑 @机器人` 时自动生成幻影牛牛（随机长度，不持久化），专属机器人文案
- **牛牛数值上下限放宽**：衰减率降低（3%→1%，2%→0.5%），硬下限解除（-100→-1M），growth 缩放基准提升（200→500），支持更大数值成长
- **Web 管理面板**：用户详情页显示击剑运势，新增设击剑运势入口，打胶/击剑运势设置增加原因输入
- **前端运势阈值适配新分布**：颜色/emoji 判定点调整为 3.0 / 1.0 / 0.3
- **单元测试**：牛牛模块 103 个测试覆盖全部子模块

### 变更

- 打胶/击剑全部数值参数化为 `NiuNiuConfig` 配置项（共 29 个 TOML 可调字段），默认值在 `games.toml.example` 中记录
- 击剑伤害浮点数精度修复：计算链中所有乘法完成后一次性 round，消除 `0.300000004` 类精度垃圾
- 长度评论文案扩展：新增深渊魅魔之神（＜-1000）和牛牛之神（＞1000）两档极端区间
...
- **打胶消息 diff 不匹配**：运势乘算后消息中的数值与实际变化不一致
- **防守方牛头人腰斩/魅魔吞噬未触发**：牛头人或魅魔作为被击剑方获胜时，特殊效果现在正确激活
- **注册时全表扫描**：`_random_initial_length` 改用 `COUNT + OFFSET` 替代全量排序
