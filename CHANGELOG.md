...
本版围绕**群成员身份认人**：艾特与各周期报告中的成员提及统一改按群级身份表渲染为标准身份，被艾特但未发言的成员档案自动注入当轮上下文，记忆、语录与留言中的数字艾特与 CQ 码残留一并清理，报告对机器人自身发言改以第一人称叙述。

**升级说明**：记忆、语录与留言新增正文片段列与成员引用索引，应用启动自动迁移数据库结构，旧正文经兼容读取照常展示、无需人工操作；如需让历史记录也按新口径展示，可执行 `python scripts/backfill_record_identities.py`（默认只读预览，确认后 `--apply` 写入，支持备份恢复）。

### 新增

- **艾特档案自动注入**：群友艾特了某位已登记成员、但对方近期没说话时，该成员的标准身份、别名与备注会自动附在当轮上下文里，模型无需对方发言即可正确认人；改名不影响识别（按 QQ 号配对）。
- 身份资料文件（`identities.yaml`）缺失、存在但为空模板、正常加载的条目数，现在都会在日志中留有记录，部署排障可直接核对。

### 变更

- **@ 提及按标准身份渲染**：消息里的艾特（含引用与合并转发内容）现在按群级身份表渲染为「@标准身份」；群级身份表此前只在发言人标签上生效，艾特与落库正文一直走全局表。
- 未登记成员被艾特时，自动查询其当前群名片并以「@名片」呈现（带缓存，查询失败才回退数字形态）。
- 模型认人规则从「QQ 号优先」改为「标准身份（名字）优先」，QQ 号仅作同名成员的区分后缀。
- 每日总结、周报、月报与每日播报中的发言者，登记成员现在以标准身份呈现（改名者不再在报告里裂成多个人）；同名不同人时自动附 QQ 号区分。
- 记忆、语录与留言统一正文检索、人物志与模型记忆读取的成员口径，提及跟随当前标准身份；管理后台支持成员引用编辑与原文查看。
- `/forget` 记忆删除新增 `#编号` 精确删除；关键词命中同名成员时不再按关键词整批删除，改为列出候选成员并提示改用 QQ 号或 `#编号` 指定。

### 修复

- **群级身份表在消息入口不生效**导致的认人失败与张冠李戴：艾特、转发、引用此前使用全局身份索引，群级登记成员在这些位置全部退化为数字或显示名。
- 修复记忆、语录和留言中的 CQ 码及数字艾特展示残留。
- 模型回复中以 `@QQ 号` 数字形态提及成员时，不再以纯文本发出，发送前自动转换为真实艾特。
- 每日总结与周报/月报对机器人自身消息的叙述改以第一人称认领：此前报告人格会把归档中带 `(bot)` 标记的自家发言当成第三方「群机器人」转述，产生引用自己发言还要「记录在案」的身份漂移；现在聊天记录格式说明明确告知撰写模型 `(bot)` 行出自其所在账号（含当前人格、其他人格与功能播报），需作为自身言行叙述。
- 修复身份资料加载器把模板占位条目当作坏数据的问题：全局身份模板自带的未填写占位条目此前会触发高频加载报错（每条消息处理的身份解析都重复失败并刷日志）；现在此类条目按不存在处理，仅当文档声明了实质条目却解析失败时才报错。
